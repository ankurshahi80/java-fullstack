...
// Typically, you'll be following CRUD operations in your DAO
public class StudentDao {

    public Student getStudentById(int id) throws SQLException {
        try (Connection con = ConnectionUtility.getConnection()) {
            String query = "SELECT * FROM students WHERE id= ?"; // ? is a placeholder for parameter

            PreparedStatement pstmt = con.prepareStatement(query);

            // Set the ? to some values
    ...
    public List<Student> getAllStudents() throws SQLException {
        ...
        try(Connection con = ConnectionUtility.getConnection()){
            ...
            // It doesn't actually contains the data inside it, it just retrieves the
            // data as we use the ResultSet object
            ResultSet rs = pstmt.executeQuery();

            // rs.next() returns a boolean, which is true if it successfully retrieves another record
            while(rs.next()){
                int id = rs.getInt("id");
                String firstName = rs.getString("first_name");
                String lastName = rs.getString("last_name");
                ...
                students.add(student); // Add to out List
            }
        }
        return students;
    }
